#!/usr/bin/env python3
"""
PATCH: Botón de factura en la sección Ventas del dashboard
============================================================
El TPV ya factura ventas; las reparaciones ya se facturan desde el
dashboard. Faltaba poder facturar ventas desde la sección Ventas del
dashboard. Este patch lo añade.

1. Botón 📄 en cada venta de la lista
2. Listener del botón
3. Función factVenta (igual que la del TPV)

Solo toca dashboard.html. Reusa abrirModalFactura (ya cargado).
"""
...
A_BTNIMPR = """    var btnImpr = '<button data-vid="' + v.id + '" class="btn-impr-v" style="background:var(--light);border:none;padding:3px 6px;border-radius:5px;font-size:11px;cursor:pointer" title="Reimprimir ticket">\U0001f5a8</button>';"""
...
ok = True
ok &= check(A_BTNIMPR, "btnImpr renderVentas")
ok &= check(A_CELDA, "celda acciones")
...
print("  ✓ botón 📄 en cada venta")

# ─── 2: celda de acciones incluye btnFact ───
dash = dash.replace(
    A_CELDA,
    "      '<td style=\"display:flex;gap:3px\">' + btnR + btnF + btnFact + btnImpr + '</td></tr>';",
    1
)
print("  ✓ botón añadido a la fila")
